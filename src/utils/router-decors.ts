...
const validateRule = paramPart => rule => {
    return function(target, name, descriptor) {
        ...
        descriptor.value = function() {
            ...
            const parameterData = new Parameter();
            console.log('ctx: ' + ctx)
            const data = paramPart === 'params' ? ctx[paramPart] : ctx.request[paramPart];
            console.log('data: ' + JSON.stringify(data));
            const errors = parameterData.validate(rule, data);
...
export const params = validateRule('params');
